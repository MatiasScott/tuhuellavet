...
class PermissionService
{
    ...
    {
        $db = Database::connection();
        $db->exec("INSERT IGNORE INTO permisos(modulo_id,accion_id,codigo,descripcion)
            SELECT ma.modulo_id,ma.accion_id,CONCAT(LOWER(m.codigo),'.',LOWER(a.codigo)),CONCAT(a.nombre,' ',m.nombre)
            FROM modulo_acciones ma JOIN modulos m ON m.id=ma.modulo_id JOIN acciones_permiso a ON a.id=ma.accion_id");
    }

    public function registerModule(string $code,string $name,array $actions=['VER','CREAR','EDITAR','ELIMINAR']): int
    {
        $db=Database::connection();$code=strtoupper(trim($code));
        $stmt=$db->prepare('INSERT INTO modulos(codigo,nombre,activo) VALUES(:c,:n,1) ON DUPLICATE KEY UPDATE nombre=VALUES(nombre),activo=1');$stmt->execute(['c'=>$code,'n'=>$name]);
        $stmt=$db->prepare('SELECT id FROM modulos WHERE codigo=:c');$stmt->execute(['c'=>$code]);$mid=(int)$stmt->fetchColumn();
        $ins=$db->prepare('INSERT IGNORE INTO modulo_acciones(modulo_id,accion_id) SELECT :m,id FROM acciones_permiso WHERE codigo=:a');foreach($actions as $a)$ins->execute(['m'=>$mid,'a'=>strtoupper($a)]);$this->syncPermissions();return$mid;
    }
}
