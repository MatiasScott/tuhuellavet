...
class AdminRepository extends Model
{
    ...
    {
        return $this->db->query('SELECT u.*,GROUP_CONCAT(DISTINCT r.nombre ORDER BY r.nombre SEPARATOR ", ") AS roles FROM usuarios u LEFT JOIN usuarios_entornos_roles uer ON uer.usuario_id=u.id LEFT JOIN roles r ON r.id=uer.rol_id WHERE u.deleted_at IS NULL GROUP BY u.id ORDER BY u.apellidos,u.nombres')->fetchAll();
    }
