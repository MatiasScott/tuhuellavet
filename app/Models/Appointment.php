...
{
    public function countToday(int $environmentId): int
    {
        $stmt = $this->db->prepare(
            'SELECT COUNT(*) FROM citas
             WHERE entorno_id=:entorno AND DATE(fecha_inicio)=CURDATE() AND deleted_at IS NULL'
        );
    ...
    public function nextToday(int $environmentId, int $limit=6): array
    {
        ...
        $stmt = $this->db->prepare(
            "SELECT c.id,c.fecha_inicio,c.motivo,a.id AS animal_id,a.nombre AS animal,
             ...
             WHERE c.entorno_id=:entorno AND DATE(c.fecha_inicio)=CURDATE() AND c.deleted_at IS NULL
             ORDER BY c.fecha_inicio LIMIT {$limit}"
        );
        $stmt->execute(['entorno' => $environmentId]);
